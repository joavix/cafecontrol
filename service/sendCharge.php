...
if ($chargeNow) {
    foreach ($chargeNow as $subscribe) {
        ...
        if ($transaction) {
            ...
            $subscribe->next_due = date("T-m-d", strtotime($subscribe->next_due . "+{$plan->period}"));
...
if ($chargeDays) {
    foreach ($chargeDays as $subscribe) {
        ...
        if ($transaction) {
            ...
            $body = $view->render("mail", [
                "subject" => $subject,
                "message" => "<h3>Que pena {$user->first_name}!</h3><p>Tentamos efetuar mais uma cobrança para sua assinatura {$plan->plan} hoje, mas sem sucesso.</p><p>Como essa já é uma segunda tentativa, infelizmente sua assinatura foi cancelada. Mas calma, você pode assinar novamente a qualquer momento.</p><p>Continue controlando com os recursos FREE, assim que quiser basta assinar novamente e voltar de onde parou.</p>"
            ]);
        }
